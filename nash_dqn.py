...
class NashDQN(DQN):
    ...
    def choose_action(self, state, Greedy=False, epsilon=None):
        ...
        if random.random() > epsilon:  # NoisyNet does not use e-greedy
            ...
            for iteration in self.action_dim:
                actions = np.random.randint(iteration, size=(state.shape[0], self.num_agents))
...
class NashDQNBase(DQNBase):
    ...
    def __init__(self, env, net_args, number_envs=2, two_side_obs=True):
        ...
        try:
            ...
            self._action_shape = (env.action_space.nvec) ** 2
        except:
            if two_side_obs:
    ...
    def _construct_net(self, env, net_args):
            ...
            output_space = gym.spaces.MultiDiscrete(self._action_shape)
