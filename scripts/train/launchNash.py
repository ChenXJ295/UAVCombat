...
def rollout(env, model, args):
    ...
    for epi in range(args.max_episodes):
        ...
        for step in range(args.max_steps_per_episode):
            ...
            if args.num_envs > 1:
                action = np.array(action_to_store).swapaxes(0,
                                                            1)  # transform from (agents, envs, dim) to (envs, agents, dim)
            else:
            ...
            if not args.test:
                ...
                if args.num_envs > 1:  # when num_envs > 1.
                    if args.marl_spec['global_state']:  # use concatenated observation from both agents
                        sample = [[states[:, j].reshape(-1), actions[:, j].reshape(-1), rewards[0, j],
                                   next_states[:, j].reshape(-1), np.any(d)] for j, d in enumerate(np.array(dones).T)]
                        ...
                        sample = [[states[0, j], actions[:, j].reshape(-1), rewards[0, j], next_states[0, j], np.any(d)]
                                  for j, d in enumerate(np.array(dones).T)]
                else:  # when num_envs = 1
                    if args.marl_spec['global_state']:
                        sample = [[np.array(states).reshape(-1), actions, rewards[0], np.array(next_states).reshape(-1),
                                   np.all(dones)]]  # done for both player
                    else:
...
# initialize the env
    ...
    eval_env = make_eval_env(args) if args.use_eval else None

# assign necessary wrappers
# if obs_type == 'rgb_image':
#     env = supersuit.max_observation_v0(env,
#                                        2)  # as per openai baseline's MaxAndSKip wrapper, maxes over the last 2 frames to deal with frame flickering
#     env = supersuit.color_reduction_v0(env, mode="B")
#     env = supersuit.frame_skip_v0(env,
#                                   4)  # skip frames for faster processing and less control to be compatable with gym, use frame_skip(env, (2,5))
#     env = supersuit.resize_v1(env, 84, 84)  # downscale observation for faster processing
#     env = supersuit.frame_stack_v1(env,
#                                    4)  # allow agent to see everything on the screen despite Atari's flickering screen problem
# else:
...
# env = supersuit.dtype_v0(env,
#                          'float32')  # need to transform uint8 to float first for normalizing observation: https://github.com/PettingZoo-Team/SuperSuit
...
#     env = supersuit.concat_vec_envs_v1(env, args.num_envs, num_cpus=0,
#                                        base_class="gym")  # true number of envs will be args.num_envs
...
#                                            # record the videos every 10000 steps
...
#     env = Dict2TupleWrapper(env)
    ...
    model = eval(args.algorithm_name)(env, args)  # NashDQN or NashDQNExploiter
