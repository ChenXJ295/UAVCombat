...
from algorithms.utils.distributions import BetaShootBernoulli, Categorical, DiagGaussian, Bernoulli
from common.actor import ACTLayer
class NetBase(nn.Module):
    ...
    def _preprocess(self, input_space, output_space):
        ...
        try:
            # self._action_shape = output_space.nvec or output_space.n # depends on different gym versions, some higher version (like 0.19) can call .shape without throwing an error; some lower version (like 0.9) will throw error for this.
            self._action_shape = output_space.nvec
...
class MLP(NetBase):
    ...
    def _construct_net(self, layers_config):
        layers = []
        for j in range(len(layers_config['hidden_dim_list']) - 2):
            tmp = [
        ...
        # action_outs = []
        # for action_dim in layers_config['hidden_dim_list'][len(layers_config['hidden_dim_list']) - 1]:
        #     action_outs.append(Categorical(layers_config['hidden_dim_list'][len(layers_config['hidden_dim_list']) - 2], action_dim, 0.9))
        # self.action_outs = nn.ModuleList(action_outs)
        # layers += self.action_outs
        print(layers_config['hidden_dim_list'][len(layers_config['hidden_dim_list']) - 2])
        return ACTLayer(act_space= self.output_space,activation_id=1,gain=0.9, mlp=nn.Sequential(*layers), input_size=layers_config['hidden_dim_list'][len(layers_config['hidden_dim_list']) - 2])
