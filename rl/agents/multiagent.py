...
import numpy as np
from mars_utils.typing import List, Union, StateType, ActionType, SampleType, SamplesType, ConfigurationDict
from nfsp_env.import_env import make_env
from .agent import Agent
from .dqn import DQN
from .ppo import PPO
from mars_marl import MetaLearner
from mars_utils.common import SelfplayBasedMethods, MetaStrategyMethods, MetaStepMethods, NashBasedMethods
        ...
        if self.args.marl_method in NashBasedMethods: 
            if self.args.exploit:  # in exploitation mode, nash policy only control one agent
                ...
                if self.args.marl_spec['global_state']:
                    ...
                    actions = self.agents[0].choose_0action(np.expand_dims(states[0], 0), Greedy=greedy_list[0])  # keep the dim unchanged
        else:
            # each agent will take its corresponding state to generate+++++++++++++++++++++++++++++++++++
            # the corresponding action
            x = zip(states, self.agents, greedy_list)
