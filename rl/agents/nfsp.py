    ...
    def choose_action(self, state, Greedy=False, epsilon=None):
        ...
        if random.random() > self.eta:
            if self.args.num_envs == 1:
                x = torch.from_numpy(np.array(state)).unsqueeze(0).float()
                y = self.policy(torch.from_numpy(np.array(state)).unsqueeze(0).float().to(self.device))
                print( y)
                print(y.squeeze())
                actions = self.policy(torch.from_numpy(np.array(state)).unsqueeze(0).float().to(self.device)).squeeze()  # make sure input state shape is correct
            else:
                actions = self.policy(torch.from_numpy(np.array(state)).float().to(self.device)).squeeze()  # make sure input state shape is correct

            # dist = Categorical(prob)
            # action = dist.sample()
            try:
                actions = actions.detach().item()
            except:
                actions = actions.detach().cpu().numpy()  # when parallel envs
        else:
            self.is_best_response = True
            actions = self.rl_agent.choose_action(state, Greedy, epsilon)
        return actions
