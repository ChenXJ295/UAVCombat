...
import logging
import envs.JSBSim.test.test_baseline_use_env as ev
...
env.max_steps = 10000
env.config.max_steps = 10000
env.task.termination_conditions[4].max_steps = 10000
env.seed(0)
args = Args()
...
for i in range(360):
    ...
    print("real reward:", rewards)
    rewards = rewards[:num_agents // 2, ...]
    print("reward:", rewards / 100)
    episode_rewards += rewards / 100
    if render:
        env.render(mode='txt', filepath=f'f.txt.acmi')
    if dones.all():
        print(infos)
        break
...
for i in range(600):
    ...
    # ac.append(ac2)
    # actions = np.concatenate((ac, enm_actions), axis=0)
    actions = np.concatenate((ego_actions, enm_actions), axis=0)
    # Obser reward and next obs
    obs, rewards, dones, infos = env.step(actions)
    print("real reward:", rewards)
...
agent0 = ev.ManeuverAgent(agent_id=0, maneuver='triangle')
agent1 = ev.PursueAgent(agent_id=1)
for i in range(600):
    action0 = agent0.get_action(env, env.task)
    action1 = agent1.get_action(env, env.task)
    actions = [action0, action1]
    obs, reward, done, info = env.step(actions)
    env.render(filepath="f.txt.acmi")
    bloods = [env.agents[agent_id].bloods for agent_id in env.agents.keys()]
    print(f"step:{env.current_step}, bloods:{bloods}")
